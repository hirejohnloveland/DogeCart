...
cart = dictionary_data.ShoppingCart()

# Driver code
if __name__ == "__main__":
    # create a GUI window
    gui = Tk()
    # new_button = form_items.Button()
    # new_button.activate()
    ...
    def refresher(command, key, qty, pr, subt):
        ...
        qty["state"] = NORMAL
        qty.delete("1.0", END)
        qty.insert("1.0", cart.get_qty(key))
        qty['state'] = DISABLED

        pr["state"] = NORMAL
        pr.delete("1.0", END)
        pr.insert("1.0", cart.get_pr(key))
        pr['state'] = DISABLED

        subt["state"] = NORMAL
        subt.delete("1.0", END)
        subt.insert("1.0", cart.item_total(key))
        ...
        total_box.insert("1.0", cart.get_total())
    ...
    qty2.insert("1.0", cart.get_qty("Tomatoes"))
    qty2['state'] = DISABLED

    #Tomatoes (pr)
    pr2 = Text(gui, height=1, width=5)
    pr2.grid(row=5, column=5)
    pr2.insert("1.0", cart.get_pr("Tomatoes"))
    pr2['state'] = DISABLED

    #Tomatoes (subt)
    subt2 = Text(gui, height=1, width=6)
    subt2.grid(row=5, column=6)
    subt2.insert("1.0", cart.item_total("Tomatoes"))
    ...
    qty4.insert("1.0", cart.get_qty("Ice Cream"))
    qty4['state'] = DISABLED

    # Ice Cream (pr)
    pr4 = Text(gui, height=1, width=5)
    pr4.grid(row=7, column=5)
    pr4.insert("1.0", cart.get_pr("Ice Cream"))
    pr4['state'] = DISABLED

    # Ice Cream (subt)
    subt4 = Text(gui, height=1, width=6)
    subt4.grid(row=7, column=6)
    subt4.insert("1.0", cart.item_total("Ice Cream"))
